...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <b>공통 레이아웃, 포인트/정산</b>
        <br/>- `Thymeleaf Layout Dialect`를 활용하여 주요 레이아웃(설정, 기본, 마이페이지)을 템플릿으로 분리 및 모듈화
        <br/>- `포트원(PortOne) API`를 사용하여 포인트 충전, 계좌 정보 검증
        <br/>- 포인트 사용 및 수익 적립: 유료 컨텐츠 구매 시 포인트 사용(구매자), 수익 적립(판매자)을 트랜잭션 단위로 관리 
        포인트 사용: 유효성 검사, 포인트 차감(구매자) 및 수익 적립(판매자) 과정을 트랜잭션으로 관리
        <br/>- 작품 구매 시 포인트 차감(구매자) 및 수익 적립(판매자): 트랜잭션 단위로 관리
        <br/>- 정산 정보 등록 시 계좌 유효성 검증: 서버 사이드에서 RestTemplate을 사용하여 PortOne API 호출
        정산 정보 등록 시 계좌 검증: CORS 문제를 해결하기 위해 `RestTemplate` 이용하여 PortOne API 호출
